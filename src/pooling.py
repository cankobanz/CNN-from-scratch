...
class Pooling(Layer):
    def __init__(self, input_shape, filter_shape, stride):  # Channels is first parameter of filter size
        ...
        self.output_shape = (self.input_depth, int((self.input_height - self.filter_height) / 2) + 1,
        ...
        self.cache = np.zeros((self.input_depth, self.input_height, self.input_height))
        ...
        for i in range(0, self.input_depth):
            for r in range(0, self.slide_times):
                for c in range(0, self.slide_times):
                    ...
                    input_slice = self.input[i, row_start: row_end, column_start:column_end]
                    self.output[i, r, c] = np.max(input_slice)
                    ...
                    self.cache[i, row_start: row_end, column_start:column_end] += truth_table
