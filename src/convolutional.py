...
class Convolutional(Layer):
    def __init__(self, input_shape, filter_size):  # Channels is first parameter of filter size
        ...
        self.output_shape = (self.channels, self.input_height - self.filter_height + 1,
        ...
        for j in range(0, self.channels):
            for r in range(0, slide_times):
                for c in range(0, slide_times):
                    input_slice = self.input[:, r: r + self.filter_height, c:c + self.filter_height]
                    self.output[j, r, c] = self.single_conv(input_slice, self.filter[j], self.bias[j, r, c])
...
# THIS PART IS THE ATTEMPT TO MAKE BACKWARD CONVOLUTION WITHOUT USING LIBRARY.
